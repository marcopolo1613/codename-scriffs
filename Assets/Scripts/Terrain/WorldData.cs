...
public class WorldData
{
    ...
    public void SetBlockTypeWithRegeneration(int x, int y, int z, BlockType blockType)
    {
        ...
        Chunks[chunkX, chunkY, chunkZ].Blocks[blockX, blockY, blockZ].Type = blockType;
	...
	//--------------------------------------------
	public void FillChunkWith(Chunk chunk, BlockType blockType)
	{
		for (int x = 0; x <= ChunkBlockWidth; x++)
            {
                for (int y = 0; y <= ChunkBlockHeight; y++)
                {
                    for (int z = 0; z < ChunkBlockDepth; z++)
                    {
                    	chunk.Blocks[x, y, z].Type = blockType; // fill each block with a given type
                    }
                }
            }
	}
