...
//                    }
                ...
                /*else*/     if (z > 1)//lowerGroundHeight)
                    {
//                        // Let's see about some caves er valleys!
//                        float octave1 = .4f;/*PerlinSimplexNoise.noise(blockX * 0.01f, blockY * 0.01f, z * 0.01f) *
//                                        (0.015f * z) + 0.1f;*/
//                        float caveNoise = .3f; /*octave1 +
...
//                        }
					...
					if(z == 0 && y == 0 && x == 0)
					{
						for (int i = 0; i < worldData.ChunkBlockDepth; i++)
            			{
							chunk.Blocks[x, y, z+i].Type = BlockType.Dirt;
						}
					}
					else
					{
